...
class PLLearner(pl.LightningModule):
    ...
    def info_nce_loss_layer(self, s_features, t_features, d=None):
        ...
        if d is not None:
            ...
            depth = 3 if self.st_inter else 1
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.t_inter:
            teacher_mid1, teacher_output1 = torch.split(teacher_output1, [batch_size * 3, batch_size], dim=0)
            teacher_mid2, teacher_output2 = torch.split(teacher_output2, [batch_size * 3, batch_size], dim=0)
        ...
        if self.st_inter:
            ...
            loss_detached = self.info_nce_loss_layer(student_detached1, teacher_output1, 4) + self.info_nce_loss_layer(student_detached2, teacher_output2, 4)
            ...
            student_mid1, student_output1 = torch.split(student_output1, [batch_size * 3, batch_size], dim=0)
            student_mid2, student_output2 = torch.split(student_output2, [batch_size * 3, batch_size], dim=0)
        ...
        loss = loss_output + self.ratio * loss_mid + 4 * loss_detached

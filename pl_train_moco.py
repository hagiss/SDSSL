...
class PLLearner(pl.LightningModule):
    def __init__(self, student, teacher, length, val_loader, embed_dim, args):
        ...
        self.pred2 = args.pred2
        self.const_ratio = args.const_ratio
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.st_inter:
            ...
            student_pred1 = self.student.predict(student_output1.detach())
            student_pred2 = self.student.predict(student_output2.detach())

            loss_2p = self.info_nce_loss(student_pred1, teacher_output1) + self.info_nce_loss(student_pred2, teacher_output2)
        ...
        loss = loss_output + self.ratio * loss_mid + 12 * self.pred_loss * loss_detached + self.pred2 * loss_2p
...
if __name__ == '__main__':
    ...
    parser.add_argument('--pred2', default=0, type=float, help="distillation token")
    parser.add_argument('--const_ratio', default=False, type=utils.bool_flag, help="distillation token")
    parser.add_argument('--pred_loss', default=1.0, type=float, help="distillation token")

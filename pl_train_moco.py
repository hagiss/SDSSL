...
class PLLearner(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.st_inter:
            ...
            loss = self.info_nce_loss_layer(student_output1, teacher_output1, 12) + self.info_nce_loss_layer(
                student_output2, teacher_output2, 12)

        else:
            student_output1 = self.student.predict(student_output1)
            student_output2 = self.student.predict(student_output2)
            loss = self.info_nce_loss(student_output1, teacher_output1) + self.info_nce_loss(student_output2, teacher_output2)
